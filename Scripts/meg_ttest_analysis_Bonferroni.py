...
"""
MEG T-Test Analysis with Bonferroni Correction
------------------
This script performs one-sample t-tests on MEG data differences between L and R conditions,
...
2. Performs one-sample t-test against zero
...
class MEGTTestBonferroniAnalyzer:
    def __init__(self):
        """Initialize the T-Test Bonferroni analyzer"""
        ...
        # Create or clean T_Test_Bonferroni directory with timestamp and divisor
        self.ttest_dir = self.dataset_dir / f"T_Test_Bonferroni_p={self.bonferroni_divisor}_{self.timestamp}"
        if self.ttest_dir.exists():
            shutil.rmtree(self.ttest_dir)
            logging.info(f"Removed existing directory: {self.ttest_dir.name}")
        
        self.ttest_dir.mkdir(parents=True)
        logging.info(f"Created directory: {self.ttest_dir.name}")
    ...
    def setup_logging(self):
        ...
        log_file = self.logs_dir / f"meg_ttest_bonferroni_analysis_{timestamp}.txt"
        ...
        logging.info("MEG T-Test Analysis with Bonferroni Correction")
        logging.info("============================================")
    ...
    def prepare_ttest_data(self):
        """Prepare data structure and L-R files for t-test analysis"""
        logging.info("Preparing data for t-test analysis...")
        ...
        for condition_dir, prefix in conditions:
            ...
            for method_freq in method_freq_dirs:
                try:
                    ...
                    method_freq_dir = self.ttest_dir / f"{condition_dir}_{method_freq}"
    ...
    def process_folder(self, folder_path: Path):
        ...
        try:
            ...
            log_file = folder_path / f"{method_freq}_ttest_Bonferroni.txt"
            with open(log_file, 'w') as f:
                f.write(f"MEG T-Test Analysis Results with Bonferroni Correction\n")
                f.write(f"============================================\n")
            ...
            # Save output matrix
            output_file = folder_path / f"{method_freq}_L-R_ttest_Bonferroni.csv"
            pd.DataFrame(output_matrix).to_csv(output_file, index=False, header=False)
            
            # Create heatmap
            heatmap_file = folder_path / f"{method_freq}_L-R_ttest_Bonferroni_heatmap.png"
            self.create_heatmap(output_matrix, heatmap_file, 
                              f"{method_freq} T-Test Bonferroni Filtered Matrix")
    ...
    def run_analysis(self):
        """Run the t-test analysis with Bonferroni correction"""
        logging.info("Starting t-test analysis with Bonferroni correction...")
        
        # First prepare the data structure and L-R files
        self.prepare_ttest_data()
...
def main():
    ...
    analyzer = MEGTTestBonferroniAnalyzer()
