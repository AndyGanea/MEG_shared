...
"""
MEG Visualization Test Script
----------------------------
This script tests the MEG visualization tool on a specific file.
It focuses on the gDTF_10Hz_L-R_wilcoxon.csv file and creates
circular connectogram visualizations with separate positive and negative connections.
"""
...
logging.info("Starting MEG Visualization Test")

# Define the test file path
test_file_path = Path("Data/DataSet5_Align_cue_626_to_938_unc/Wilcoxon_03102025-1305/Cue_gDTF_10Hz/gDTF_10Hz_L-R_wilcoxon.csv")
...
def read_test_file():
    """Read the test CSV file and create a matrix"""
    logging.info(f"Reading test file: {test_file_path}")
...
def run_test():
    ...
    # Read the test file
    matrix = read_test_file()
    
    # Generate timestamp for filenames
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # Output folder is the same as the input file
    output_folder = test_file_path.parent
    
    # Method and frequency from the filename
    method_freq = "gDTF_10Hz"
    
    # Generate connectivity report
    report_path = output_folder / f"{method_freq}_L-R_wilcoxon_connectivity_report_TEST_{timestamp}.txt"
    ...
    pos_title = f"{method_freq} Wilcoxon\nPositive Connectivity"
    pos_output_path = output_folder / f"{method_freq}_L-R_wilcoxon_circular_connectogram_positive_TEST_{timestamp}.png"
    ...
    pos_fig.savefig(pos_output_path, dpi=300, bbox_inches='tight', facecolor='black')
    ...
    neg_title = f"{method_freq} Wilcoxon\nNegative Connectivity"
    neg_output_path = output_folder / f"{method_freq}_L-R_wilcoxon_circular_connectogram_negative_TEST_{timestamp}.png"
    ...
    # Save the PNG version with black background
    neg_fig.savefig(neg_output_path, dpi=300, bbox_inches='tight', facecolor='black')
