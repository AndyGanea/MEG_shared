...
class MEGWilcoxonAnalyzer:
        ...
        for condition_dir, prefix in conditions:
            ...
            for method_freq in method_freq_dirs:
                try:
                    ...
                    # Check for NO-LT files if user requested them
                    if self.use_no_lt_files:
                        no_lt_exists_l = self.check_no_lt_files_exist(l_base_dir, prefix, method_freq)
                        no_lt_exists_r = self.check_no_lt_files_exist(r_base_dir, prefix, method_freq)
                        
                        if no_lt_exists_l and no_lt_exists_r:
                            logging.info(f"NO-LT files found for {method_freq}, using NO-LT averages")
                        else:
                            logging.warning(f"NO-LT files not found for {method_freq}, falling back to regular averages")
                            logging.info(f"L directory has NO-LT files: {no_lt_exists_l}")
                            logging.info(f"R directory has NO-LT files: {no_lt_exists_r}")
                    ...
                    for subject in subject_folders:
                        ...
                        # If NO-LT files not found and user requested them, try regular files
                        if self.use_no_lt_files and (l_file is None or r_file is None):
                            logging.info(f"NO-LT files not found for subject {subject}, trying regular files")
                            
                            # Get regular file patterns (not NO-LT)
                            if subject:
                                regular_l_patterns = [
                                    f"{prefix}_L_{method_freq}_{subject}_average.csv",
                                    f"{prefix}_L_{method_freq}_average.csv"
                            ...
                            # Override patterns to use regular files
                            if l_file is None:
                                for l_pattern in regular_l_patterns:
                                    found = list(l_base_dir.joinpath(subject).glob(l_pattern))
                                    if found:
                                        l_file = found[0]
                                        logging.info(f"Found regular L file for subject {subject}: {l_file.name}")
                                        break
                            
                            if r_file is None:
                                for r_pattern in regular_r_patterns:
                                    found = list(r_base_dir.joinpath(subject).glob(r_pattern))
                                    if found:
                                        r_file = found[0]
                                        logging.info(f"Found regular R file for subject {subject}: {r_file.name}")
                                        break
    ...
    def process_folder(self, folder_path: Path):
        ...
        try:
            ...
            heatmap_file = folder_path / heatmap_filename
    ...
    def has_sufficient_data(self, folder_path: Path, condition: str, method_freq: str) -> bool:
        ...
        # Add NO-LT patterns if user selected them
        if self.use_no_lt_files:
            no_lt_patterns = [
                f"{condition}_L_{method_freq}_average_NO-LT.csv",
                f"{method_freq}_L-R_overall_NO-LT.csv"
            ]
            overall_patterns.extend(no_lt_patterns)
